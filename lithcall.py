...
############################################################################
# Func: the effect of temp. and pressure on density is calculated altogether.
############################################################################
...
def density_TP(T_new0, T_old0, P_new0, P_old0,rho_old0):#(new_temp, old_temp, old_rho):
    ...
    for i in range(mx):
        for j in range(my):
            ...
            if (xx[j, i] >= 0 and xx[j, i] <= w
                ...
                rho_TP[j, i] = rho_old0[j, i] + drhodT_lith * \
                    (T_new0[j, i] - T_old0[j, i])   \
                 + drhodP_asth * (P_new0[j, i] - P_old0[j, i])
            
            # Lithospheric mantle layer
            if (xx[j, i] >= 0 and xx[j, i] <= w
                    and yy[j, i] >= box_moho and yy[j, i] <= dlab):
                rho_TP[j, i] = rho_old0[j, i] + drhodT_lith * \
                    (T_new0[j, i] - T_old0[j, i])  \
                 + drhodP_lith * (P_new0[j, i] - P_old0[j, i])
            # Triangular block (joint)
            if (yy[j, i] >= box_moho and yy[j, i] <= dlab
                    and xx[j, i] >= hinge_ax - (yy[j, i] - box_moho) / gradient1
                    and xx[j, i] <= hinge_ax + (yy[j, i] - box_moho) / gradient2):
                rho_TP[j, i] = rho_old0[j, i] + drhodT_lith * \
                    (T_new0[j, i] - T_old0[j, i])   \
                 + drhodP_lith * (P_new0[j, i] - P_old0[j, i])
            # Hanging lithospheric slab
            if (yy[j, i] >= dlab and yy[j, i] <= dbottom
                    and xx[j, i] <= hinge_bx + wid + (yy[j, i] - dlab) / gradient2
                    and xx[j, i] >= hinge_bx + (yy[j, i] - dlab) / gradient2):
                rho_TP[j, i] = rho_old0[j, i] + drhodT_lith * \
                    (T_new0[j, i] - T_old0[j, i])   \
                 + drhodP_lith * (P_new0[j, i] - P_old0[j, i])
...
#########################################################################
def advdiff(temp,dens,P,vx,vy,dx,dy,dt):
    dfdt= np.zeros(np.shape(xx))
    dfdt_adv_dens= np.zeros(np.shape(xx))
    dfdt_adv_P= np.zeros(np.shape(xx))
    kappa = np.zeros(np.shape(xx))
    ...
    kappa[np.int(box_moho / dy):np.int(slab_thickness / dy) + 1, :] = 1e-6#kappa_lith#k_lith / (dens[np.int(box_moho / dy):np.int(dlab / dy) + 1, :] * Cp)
    kappa[np.int(slab_thickness / dy) + 1::, :] = 1e-6#kappa_asth#k_asth / (dens[np.int(dlab / dy) + 1::, :] * Cp)

    ################### Diffusion #################
    #   Internal diffusion:
    dfdt[1:-1, 1:-1] = kappa[1:-1,1:-1]*(Txx[:,:]+Tyy[:,:])
    #   Natural b.c.'s at side boundaries:
    dfdt[1:-1, 0] = dfdt[1:-1,0]+2*kappa[1:-1,0]*(temp[1:-1,1]- temp[1:-1,0])/dx2
    dfdt[1:-1,-1] = dfdt[1:-1,-1]+2*kappa[1:-1,0]*(temp[1:-1,-2]-temp[1:-1,-1])/dx2
    ...
    for i in range(1, mx - 1):
        for j in range(0, my):
            if vx[j, i] > 0:
                dfdtx_dens = vx[j, i] * (dens[j, i - 1] - dens[j, i]) / dx
                dfdtx = vx[j, i] * (temp[j, i - 1] - temp[j, i]) / dx
                dfdtx_P = vx[j, i] * (P[j, i - 1] - P[j, i]) / dx
            else:
                dfdtx_dens = vx[j, i] * (dens[j, i] - dens[j, i + 1]) / dx
                dfdtx = vx[j, i] * (temp[j, i] - temp[j, i + 1]) / dx
                dfdtx_P = vx[j, i] * (P[j, i] - P[j, i + 1]) / dx
            dfdt_adv_dens[j, i] = dfdtx_dens
    ...
    for i in range(0, mx):
        for j in range(1, my - 1):
            if vy[j, i] > 0:
                dfdty_dens = vy[j, i] * (dens[j - 1, i] - dens[j, i]) / dy
                dfdty = vy[j, i] * (temp[j - 1, i] - temp[j, i]) / dy
                dfdty_P = vy[j, i] * (P[j - 1, i] - P[j, i]) / dy
            else:
                dfdty_dens = vy[j, i] * (dens[j, i] - dens[j + 1, i]) / dy
                dfdty = vy[j, i] * (temp[j, i] - temp[j + 1, i]) / dy
                dfdty_P = vy[j, i] * (P[j, i] - P[j + 1, i]) / dy                
            dfdt_adv_dens[j, i] = dfdt_adv_dens[j, i] + dfdty_dens
            dfdt[j, i] = dfdt[j, i] + dfdty
            dfdt_adv_P[j,i] = dfdt_adv_P[j,i] + dfdty_P

    # Add dt*df/dt-vector to old solution:
    rho_out = dens + dt * dfdt_adv_dens
    T_out = temp + dt * dfdt
    P_out = P + dt * dfdt_adv_P

    return [T_out, rho_out, P_out]
...
#            ax2 = plt.subplot(gs[3:6, 6:])  # New density
            ...
            # ax1.plot((slice_sec1 * dx, slice_sec1 * dx),
            #          (0, h), 'k--', alpha=0.4)
            # ax2.plot((slice_sec1 * dx, slice_sec1 * dx),
            #          (0, h), 'k--', alpha=0.4)
#            ax2.plot(x_plot,y_plot,'k--', alpha=0.4)
            # ax5.plot((slice_sec1 * dx, slice_sec1 * dx),
            #          (0, h), 'k--', alpha=0.4)
