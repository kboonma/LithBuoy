...
fig_path= os.getcwd()+'/data/%s_%s_vel%d/subplots/' % (params.experiment_number,mantle_type,params.velocity)
dat_path= os.getcwd()+'/data/%s_%s_vel%d/' % (params.experiment_number,mantle_type,params.velocity)
...
for i in fnum:
    ...
    ax1.set_title(mantle_type+' T, t=' + str("%.2f" % tmyrs) + ' Myr, tstep =' +
                  str(i) + ', v=' + str(params.velocity) + '$mm$ $yr^{-1}$'+
    ...
    os.chdir(dir1)
    plt.savefig(str(params.experiment_number) + str(mantle_type) +'_subplots' + str(i) + '.png', format='png', dpi=300)
